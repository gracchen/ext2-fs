...
void write_superblock(int fd) {
	...
	superblock.s_inodes_count      = NUM_INODES; //max capacity is 128
	superblock.s_blocks_count      = NUM_BLOCKS; //max capacity of 1024
	superblock.s_r_blocks_count    = 0;          //1st 1024 bytes reserved (boot block)???? 
	superblock.s_free_blocks_count = NUM_FREE_BLOCKS; //blocks - last block - 1 (starts at 0)
	superblock.s_free_inodes_count = NUM_FREE_INODES;  //inodes - last inode
	superblock.s_first_data_block  = SUPERBLOCK_BLOCKNO; /* First Data Block */ //superblock # = 1
	superblock.s_log_block_size    = 0; /* 1024 */
	superblock.s_log_frag_size     = 0; /* 1024 */
	superblock.s_blocks_per_group  = 8*BLOCK_SIZE; //max capacity of ONE block bitmap = block size = 1024 bytes = 8192 bits
	superblock.s_frags_per_group   = 8*BLOCK_SIZE;
	superblock.s_inodes_per_group  = NUM_INODES; //inode table @ # 5-20 (16 blocks). only 1 group, so num_inodes = inodes_per_group 
	superblock.s_mtime             = 0; /* Mount time */ //BUT science.smith.edu says its MOD time
...
void write_block_bitmap(int fd) {
	...
	if (off == -1) errno_exit("lseek");

	unsigned char bmp[BLOCK_SIZE] = {0}; //initialize to all zeros
	
	for (int i = 1; i <= LAST_BLOCK; i++)
	{
	  int n = (i%8==0)? 8 : i - (i/8) *8; //as if 1st row	  
	  n--; //now its the weight of power of 2
	  int m = (i - 1) / 8;       
	  bmp[m] += 1 << n;
	  }

	//extra blocks in bitmap that don't exist
	...
	if (write(fd, &bmp, BLOCK_SIZE) != BLOCK_SIZE) errno_exit("write");
...
void write_inode_bitmap(int fd) {
	...
	{
	  int n = (i%8==0)? i=8 : i - (i/8) *8; //as if 1st row	  
	  n--; //now its the weight of power of 2
	  int m = (i - 1) / 8;       
	  bmp[m] += 1 << n;
	}
	
	//bmp[0] = 0xff; hardcode versions
	//bmp[1] = 0x1f;
	//extra inodes in bitmap that don't exist
...
void write_inode_table(int fd) {
	...
	//root dir:   rwx-rx-rx  , owner=USR, groug=GRP, other=OTH   variable format: EXT2_S_I <R/W/X> <USR/GRP/OTH>
	...
	root_inode.i_gid = 0;
	root_inode.i_links_count = 3; //. and .. in root dir entry +  .. in lost+found dir entry
	...
	char str[11] = "hello-world";
        memset(hello_inode.i_block, 0, sizeof hello_inode.i_block);
	memcpy(hello_inode.i_block, str, strlen(str));
...
void write_hello_world_file_block(int fd) {
	...
	if (write(fd, &str, BLOCK_SIZE) != BLOCK_SIZE) errno_exit("write");
}
